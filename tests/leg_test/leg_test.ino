...
// Servo 3
#define SERVO3_PIN 50
#define SERVO3_DEFAULT 90
#define SERVO3_MIN 9
#define SERVO3_MAX 163
// LED 1
#define LED1_PIN 52
...
void loop() {
  ...
  if (transmitter_active) {
    ...
    float percentage = (float)(getChannelValue(ch1Status, ch1Counter) - CH_STICK_MIN) / (CH_STICK_MAX - CH_STICK_MIN);
    ...
    // Translate the current value to a percentage and change it to a valid value (>= 0 & <= 100)
    percentage = (float)(getChannelValue(ch2Status, ch2Counter) - CH_STICK_MIN) / (CH_STICK_MAX - CH_STICK_MIN);
    if (percentage > 100.0) { percentage = 100.0; }
    if (percentage < 0.0) { percentage = 0.0; }
    // Calculate angle based on percentage
    ...
    if (percentage < 0.0) { percentage = 0.0; }
    // Calculate angle based on percentage
    angle = (int)round(SERVO3_MIN + percentage * (SERVO3_MAX - SERVO3_MIN));
    servoWrite(servo3, angle);
